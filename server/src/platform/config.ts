...
const EnvSchema = z.object({
  ...
  // Per-section prompt metrics are useful locally, but stay compact in every
  // deployed/CI environment. Empty (as shipped in .env.example) means off.
  PROMPT_LOG_VERBOSE: z.preprocess(
    (v) => (v === '' ? undefined : v),
    z.enum(['0', '1']).optional(),
  ),
...
export type AppConfig = {
  ...
  /** Per-section prompt metrics; hard-disabled outside local development. */
...
export function loadConfig(env: NodeJS.ProcessEnv = process.env): AppConfig {
  ...
  return {
    ...
    promptLogVerbose:
      parsed.NODE_ENV === 'development' && parsed.PROMPT_LOG_VERBOSE === '1',
  };
}
