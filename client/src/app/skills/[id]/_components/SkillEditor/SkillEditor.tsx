...
/** Skill Editor — Config/Preview/Stats/Versions tabs. Tab state lives in ?tab=
 *  (owned by the page, not this component), mirroring the Agent Editor. */
export function SkillEditor({ skill, tab, onTab }: { skill: Skill; tab: string; onTab: (t: string) => void }) {
  const t = useTranslations("skills");
  const tabs = TABS.map((tb) => ({ key: tb.key, label: t(tb.labelKey), icon: tb.icon }));

  let content: React.ReactNode;
  if (tab === "preview") content = <PreviewTab skill={skill} />;
