...
| `brief` | Full split | routes, service, repository, state-key, signals, budget | Coordinates many data sources (`IntentService`, `BlastService`, `reviewRepo.getPrFiles`/`getPrCommits`, `container.github()`, `contextDocs.resolveForAgent`/`statBodies`/`readBodies`, `repoIntel.getIndexState`) to compute a composite cache state key (`state-key.ts`), gather LLM-prompt sections and derive accepted-citation file sets (`signals.ts`), and iteratively trim to a token budget (`budget.ts`) before generating and persisting a Brief — `service.ts` is the sole consumer of `repository.ts` |
| `eval` | Full split | routes, service, repository, scorer, dashboard, run-tracker, constants, errors | Coordinates the review engine, agents, skills, and its own tables — same graduated-layering shape as blast/smart-diff |
